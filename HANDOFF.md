...
| 位姿闭环 | 眼镜 POSE 通道供数 + 相机注册 AR 叠加 + 实时跟随导航 | 🟡 第 1 档 ✅（POSE 消费链路 + pose.csv 同源）；第 2 档第 1 阶段 ✅（跟随视角：heading-up 小地图 + 用户箭头 + 实时相对转向播报，单测覆盖纯数学）；第 2 档第 2 阶段 ✅（IMU 通道 + StepDetector 步态检测 + 位置递推 + 到达自动播报收尾，纯数学单测覆盖）；剩余：相机 AR 叠加、步长自适应（漂移大时引入步频/身高先验）。**双源步态检测已完成（2026-09-03 真机标定+实施：手机源主/眼镜源降级，真值验证检出率 ~100-109%/102%，见 §6）** |
| **P0：前台服务化** | 熄屏口袋模式存活（盲人用户主场景） | 🔴 **缺口已确认（2026-09-03）**：整条采集链路跑在 Activity 进程内，manifest 无任何 `<service>`；WAKE_LOCK 权限已声明但代码零调用（M0 教训：不持锁被系统 suspend，3 分钟编码拉长到 401s）。实测：熄屏揣兜 ~2 分钟存活（双源标定第 2 轮即此口径），但 5-15 分钟后 Doze 冻结 + Android 11+ 后台麦克风静默切断（语音命令/VAD 失效）+ 三星 One UI 激进杀后台。**修复方向**：常驻前台服务 `foregroundServiceType="microphone|dataSync"` + 启动持 PARTIAL_WAKE_LOCK + 常驻通知（可兼状态播报），采集管线生命周期从 Activity 挪到 Service。**2026-09-04 真机复现实锤（锁屏黑屏事故）**：熄屏后 Wi-Fi Direct 组被系统拆除（19:00:26 DISCONNECTED）→ TCP 会话结束 → 解码器释放 → app 侧被动 listen 永远等不到重连（无前台服务驱动探测、无 WifiLock 保 P2P、眼镜端退避重连因 P2P 网络不存在而无效）；进程未死（TTS 照播）但链路全断、亮屏后不自愈。修复须补：前台服务内持 WifiLock（WIFI_MODE_FULL_HIGH_PERF）+ P2P 重连驱动 + 链路级健康探测（IMU/帧流静默超阈值即触发重连），不能只靠被动 listen |

## 6. 已知问题与重要决策

### 已知问题/边界
- **无实时位姿（大部分解决）**：眼镜 POSE + IMU 已接入，位置递推驱动小地图与到达判定；剩余相机 AR 叠加未做。递推步长固定 0.7m（无绝对尺度参考），长距离行走会漂移，待真机联调评估是否引入步长自适应。
- **StepDetector 真机标定完成（2026-09-03，眼镜 X3 Pro + 手机 SM_F711N 双源配对）**：真机眼镜会话经 Wi-Fi Direct 链路采集，眼镜 IMU 新落盘 `glasses_imu.csv`（与手机 imu.csv 分开，手机传感器在眼镜模式下仍写 imu.csv）。真值口径：用户默数 110-120 步。结论：
  1. **双源幅值差一个量级**：手机兜里 A_p95≈2.8-3.6（随腿摆）、眼镜头动 A_p95≈0.9-1.1。第 1 轮（未带手机）眼镜源"53% 检出"是 FFT 真值高估假象（假设连续行走，实际走走停停）；带真值第 2 轮：**MIN_THRESHOLD=0.6 保持不动**（手机 100-109%、眼镜 102%）。
  2. **阈值悬崖复核**：眼镜源 th≥0.8 检出骤降（83→61→37@1.0-1.2），0.6 恰在悬崖上沿；手机源 th=0.3-1.2 全程稳定（123-126）。
  3. **融合并集过计**：两源 300ms 去重并集 133 步 vs 真值 110-120（~121%），不采用。
  4. **静止佩戴虚警**：眼镜 2 步/30s、手机 0 步/30s，可接受。
  5. **已实施（双源架构，两客户端同步）**：`MemoryLearningCoordinator` 双 `StepDetector` + 源选择——手机源为主（`PHONE_FRESH_WINDOW_NS=1s` 内有手机样本即忽略眼镜样本），眼镜源降级（手机流断超窗口生效）；`shouldAcceptGlassesImu` 纯函数 + `ImuSourceSelectionTest` 单测；发起导航时双检测器同复位。手机 IMU 回调（CaptureCoordinator sensorSource.onImu）接入 `updateUserImu`（原来只喂视觉管线）。
  6. **遗留**：眼镜端链路支持无线（Wi-Fi Direct 192.168.49.x 自动重连）与 USB 隧道两种；步长仍固定 0.7m 待漂移评估；不应期 250ms 下手机源 ~105-115% 略过计（450ms 降至 100-109%，因真值区间不精确未动，后续可再校）。
